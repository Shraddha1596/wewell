		...
		class Unicon_Lite_Welcome {
			...
			public function __construct() {
				...
				$this->tab_sections = array(
					...
					'more_wp_stuff' => __('More WordPress Stuff', 'unicon-lite'),
				);
					...
					<div class="notice notice-success is-dismissible">
						<p>
							<?php
								...
								printf( wp_kses(__( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'unicon-lite' ), array( 'a' => array( 'href' => array() ) ) ), esc_html($this->theme_name), esc_url( admin_url( 'themes.php?page=unicon-lite-welcome' ) )  );
							?>
						</p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=unicon-lite-welcome' )); ?>"><?php esc_html_e( 'Lets Get Started', 'unicon-lite' ); ?></a></p>
					</div>
					<?php
				}
			}
				...
				<div class="wrap about-wrap access-wrap">
						...
						<div class="abt-theme-wrap">
							<h1>
								<?php
									...
									printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'unicon-lite' ), esc_html($this->theme_name), esc_html($this->theme_version) );
								?>
							</h1>
							<div class="about-text">
								<?php
									/* translators: %s : theme name */
									printf( esc_html__( 'The %s is a creative theme with elegant design perfect for business, corporate, photography, personal blog and other creative website. The theme has creative design with vibrant color and all the required sections to make a complete website. The theme is fully translation ready and built with customizer for easy setup with live preview. The Theme has creative team, testimonial, service, video and portfolio section.', 'unicon-lite' ), esc_html($this->theme_name) );
								?>
							</div>
						</div>
					...
					<div class="nav-tab-wrapper clearfix">
						<?php foreach($tabs as $id => $label) : ?>
							...
							?>
							<a href="<?php echo esc_url(admin_url('themes.php?page=unicon-lite-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function unicon_lite_plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'unicon-lite' ) );
				...
				if (! wp_verify_nonce( $nonce, 'unicon_lite_plugin_installer_nonce' ))
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'unicon-lite') );
			...
			public function unicon_lite_plugin_activation_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'unicon-lite' ) );
				...
				if (! wp_verify_nonce( $nonce, 'unicon_lite_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'unicon-lite' ) );
