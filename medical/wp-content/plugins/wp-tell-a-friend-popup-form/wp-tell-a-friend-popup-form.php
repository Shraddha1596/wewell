...
Text Domain: wp-tell-a-friend-popup-form
...
function TellAFriend()
{
	...
	<div style="display:block !important"  id="TellAFriend_BoxContainer">
	  <div id="TellAFriend_BoxContainerHeader">
		<div id="TellAFriend_BoxTitle"><?php echo get_option('TellAFriend_Title'); ?></div>
		<div id="TellAFriend_BoxClose"><a href="javascript:TellAFriend_HideForm('TellAFriend_BoxContainer','TellAFriend_BoxContainerFooter');"><?php _e('Close', 'wp-tell-a-friend-popup-form'); ?></a></div>
	  </div>
	  <div id="TellAFriend_BoxContainerBody">
		<form action="#"  name="TellAFriend_Form" id="TellAFriend_Form">
		  ...
		  <div id="TellAFriend_BoxLabel"> <?php _e('Your Name', 'wp-tell-a-friend-popup-form'); ?> </div>
		  ...
		  <div id="TellAFriend_BoxLabel"> <?php _e('Friend Email', 'wp-tell-a-friend-popup-form'); ?> </div>
		  <div id="TellAFriend_BoxLabel">
			<input name="TellAFriend_email" class="TellAFriend_TextBox" type="text" id="TellAFriend_email" maxlength="120">
		  </div>
		  <div id="TellAFriend_BoxLabel"> <?php _e('Enter Message To Friend', 'wp-tell-a-friend-popup-form'); ?> </div>
		  <div id="TellAFriend_BoxLabel">
...
{
	echo '<p>';
	_e('Check official website for more information', 'wp-tell-a-friend-popup-form');
	?> <a target="_blank" href="http://www.gopiplus.com/work/2012/05/21/wordpress-plugin-wp-tell-a-friend-popup-form/"><?php _e('click here', 'wp-tell-a-friend-popup-form'); ?></a></p><?php
}
...
function TellAFriend_widget_init()
{
	...
	{
		wp_register_sidebar_widget( __('Tell A Friend', 'wp-tell-a-friend-popup-form'), __('Tell A Friend', 'wp-tell-a-friend-popup-form'), 'TellAFriend_widget');
	}
	
	if(function_exists('wp_register_widget_control')) 
	{
		wp_register_widget_control( __('Tell A Friend', 'wp-tell-a-friend-popup-form'), array( __('Tell A Friend', 'wp-tell-a-friend-popup-form'), 'widgets'), 'TellAFriend_control');
	} 
}
...
function TellAFriend_admin()
{
	...
	<h2><?php _e('Tell a friend popup form', 'wp-tell-a-friend-popup-form'); ?></h2>
	<?php
	global $wpdb, $wp_version;
	...
	{
		$TellAFriend_Title = stripslashes(sanitize_text_field($_POST['TellAFriend_Title']));
		$TellAFriend_Fromemail = stripslashes(sanitize_text_field($_POST['TellAFriend_Fromemail']));
		$TellAFriend_On_MyEmail = stripslashes(sanitize_text_field($_POST['TellAFriend_On_MyEmail']));
		$TellAFriend_On_Subject = stripslashes(sanitize_text_field($_POST['TellAFriend_On_Subject']));
		$TellAFriend_Caption = stripslashes(wp_filter_post_kses($_POST['TellAFriend_Caption']));
		$TellAFriend_Adminmail_Content = stripslashes(wp_filter_post_kses($_POST['TellAFriend_Adminmail_Content']));
		$TellAFriend_Usermail_Content = stripslashes(wp_filter_post_kses($_POST['TellAFriend_Usermail_Content']));
		$TellAFriend_homeurl = stripslashes(sanitize_text_field($_POST['TellAFriend_homeurl']));
		...
		<div class="updated fade">
			<p><strong><?php _e('Details successfully updated.', 'wp-tell-a-friend-popup-form'); ?></strong></p>
		</div>
		<?php
	}
	echo '<h3>'.__('Plugin setting', 'wp-tell-a-friend-popup-form').'</h3>';
	echo '<form name="form_gCF" method="post" action="">';
	
	echo '<label for="tag-title">'.__('Title', 'wp-tell-a-friend-popup-form').'</label><input  style="width: 350px;" type="text" value="';
	echo $TellAFriend_Title . '" name="TellAFriend_Title" id="TellAFriend_Title" /><p>'.__('Enter your popup box title.', 'wp-tell-a-friend-popup-form').'</p>';
	
	echo '<label for="tag-title">'.__('From email:', 'wp-tell-a-friend-popup-form').'</label><input  style="width: 350px;" type="text" value="';
	echo $TellAFriend_Fromemail . '" name="TellAFriend_Fromemail" id="TellAFriend_Fromemail" /><p>'.__('Enter From email address for your mail.', 'wp-tell-a-friend-popup-form').'</p>';
	
	echo '<label for="tag-title">'.__('Admin email:', 'wp-tell-a-friend-popup-form').'</label><input style="width: 350px;" type="text" value="';
	echo $TellAFriend_On_MyEmail . '" name="TellAFriend_On_MyEmail" maxlength="200" id="TellAFriend_On_MyEmail" /><br />';
	echo '<p>'.__('Enter admin email address to receive  Tell A Friend Mail copy.', 'wp-tell-a-friend-popup-form').'</p>';
	
	echo '<label for="tag-title">'.__('Email subject:', 'wp-tell-a-friend-popup-form').'</label><input style="width: 350px;" type="text" value="';
	echo $TellAFriend_On_Subject . '" name="TellAFriend_On_Subject" maxlength="200" id="TellAFriend_On_Subject" /><br />';
	echo '<p>'.__('Enter mail subject.', 'wp-tell-a-friend-popup-form').'</p>';
	
	echo '<label for="tag-title">'.__('Link button/text:', 'wp-tell-a-friend-popup-form').'</label><input style="width: 800px;" type="text" value="';
	echo $TellAFriend_Caption . '" name="TellAFriend_Caption" id="TellAFriend_Caption" /><br />';
	echo '<p>'.__('This box is to add the contact us Image or text, Entered value will display in the front end.', 'wp-tell-a-friend-popup-form').'</p>';
	
	?>
    <table width="640" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><label for="tag-title"><?php _e('Admin mail content:', 'wp-tell-a-friend-popup-form'); ?></label></td>
        <td><label for="tag-title"><?php _e('User mail content:', 'wp-tell-a-friend-popup-form'); ?></label></td>
      </tr>
      <tr>
	...
	<h3><?php _e('Security Check (Spam Stopper)', 'wp-tell-a-friend-popup-form'); ?></h3>
	<label for="tag-width"><?php _e('Home URL', 'wp-tell-a-friend-popup-form'); ?></label>
	<input name="TellAFriend_homeurl" type="text" value="<?php echo $TellAFriend_homeurl; ?>"  id="TellAFriend_homeurl" size="50" maxlength="500">
	<p><?php _e	('This home URL is for security check. We can submit the form only on this website. ', 'wp-tell-a-friend-popup-form'); ?></p>
	
	<div style="height:8px;"></div>
	<input type="submit" id="TellAFriend_submit" name="TellAFriend_submit" lang="publish" class="button add-new-h2" value="<?php _e('Update Setting', 'wp-tell-a-friend-popup-form'); ?>" />
	<input name="Help" lang="publish" class="button add-new-h2" onclick="window.open('http://www.gopiplus.com/work/2012/05/21/wordpress-plugin-wp-tell-a-friend-popup-form/');" value="<?php _e('Help', 'wp-tell-a-friend-popup-form'); ?>" type="button" />
	</form>
	</div>
	<h3><?php _e('Plugin configuration option', 'wp-tell-a-friend-popup-form'); ?></h3>
	<ol>
		<li><?php _e('Drag and drop the plugin widget to your sidebar.', 'wp-tell-a-friend-popup-form'); ?></li>
		<li><?php _e('Add plugin in the posts or pages using short code.', 'wp-tell-a-friend-popup-form'); ?></li>
		<li><?php _e('Add directly in to the theme using PHP code.', 'wp-tell-a-friend-popup-form'); ?></li>
	</ol>
	<p class="description">
		<?php _e('Check official website for more information', 'wp-tell-a-friend-popup-form'); ?> 
		<a target="_blank" href="http://www.gopiplus.com/work/2012/05/21/wordpress-plugin-wp-tell-a-friend-popup-form/"><?php _e('click here', 'wp-tell-a-friend-popup-form'); ?></a>
	</p>
	</div>
	<?php
}
...
{
	add_options_page(__('Tell A friend', 'wp-tell-a-friend-popup-form'), __('Tell A friend', 'wp-tell-a-friend-popup-form'), 'manage_options', __FILE__, 'TellAFriend_admin' );
}
...
function TellAFriend_add_javascript_files() 
{
	if (!is_admin())
	{
		wp_enqueue_style( 'wp-tell-a-friend-popup-form', get_option('siteurl').'/wp-content/plugins/wp-tell-a-friend-popup-form/tell-a-friend.css');
...
function TellAFriend_shortcode( $atts ) 
{
	...
	$html .= '<div style="display: block;" id="TellAFriend_BoxContainer">';
	  $html .= '<div id="TellAFriend_BoxContainerHeader">';
		$html .= '<div id="TellAFriend_BoxTitle">'.$PopupContact_title.'</div>';
		$html .= '<div id="TellAFriend_BoxClose"><a href="'.$close .'">'.__('Close', 'wp-tell-a-friend-popup-form').'</a></div>';
	  $html .= '</div>';
	  $html .= '<div id="TellAFriend_BoxContainerBody">';
		$html .= '<form action="#" name="TellAFriend_Form" id="TellAFriend_Form">';
			...
			// $html .= '<div id="TellAFriend_BoxLabel_Page"> '.__('Friend Email', 'wp-tell-a-friend-popup-form').' </div>';
		  $html .= '<div id="TellAFriend_BoxLabel_Page">';
			$html .= '<input name="TellAFriend_email" type="text" id="TellAFriend_email" class="TellAFriend_TextBox" placeholder="Email Address" maxlength="120">';
		  $html .= '</div>';
		  // $html .= '<div id="TellAFriend_BoxLabel_Page"> '.__('Your Name', 'wp-tell-a-friend-popup-form').' </div>';
		  $html .= '<div id="TellAFriend_BoxLabel_Page">';
			$html .= '<input name="TellAFriend_name" class="TellAFriend_TextBox" placeholder="Aaron Albright wants to introduce you to WeWell" type="text" id="TellAFriend_name" maxlength="120">';
		  $html .= '</div>';
		  
		  // $html .= '<div id="TellAFriend_BoxLabel_Page"> '.__('Enter Message To Friend', 'wp-tell-a-friend-popup-form').' </div>';
		  $html .= '<div id="TellAFriend_BoxLabel_Page">';
			$html .= '<textarea name="TellAFriend_message" class="TellAFriend_TextArea" rows="3" id="TellAFriend_message">&#13;&#10; Have I told you about WeWell yet?&#13;&#10;&#10; It’s a social healthcare platform for holistic and integrative practitioners to gain more clients, establish their brand and connect with other practitioners for co-referrals.&#13;&#10;&#10; WeWell is offering free early access to key select practitioners and I thought about you. If you accept my invitation, we’ll both  get $25 in credit and skip the line for early access.&#13;&#10;&#10;Be well</textarea>';
		  $html .= '</div>';
		  $html .= '<div id="TellAFriend_BoxLabel_Page">';
			$html .= '<input type="button" name="button" class="TellAFriend_Button" value="Send Invite" onClick="javascript:TellAFriend_Submit(this.parentNode,'.$siteurl.');">';
		  $html .= '</div>';
...
{
	  load_plugin_textdomain( 'wp-tell-a-friend-popup-form', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
}
...
function TellAFriend_plugin_parse_request($qstring)
{
	if (array_key_exists('tellafriend', $qstring->query_vars)) 
	{
		...
		{
			case 'send-mail':
				$ToEmail = isset($_POST['TellAFriend_email']) ? sanitize_text_field($_POST['TellAFriend_email']) : '';
				// echo $ToEmail.' is the mail';
				// die('xxxx');
				if($ToEmail <> "")
				{
					...
					else
					{
						...
						{
							$SenderName = isset($_POST['TellAFriend_name']) ? sanitize_text_field($_POST['TellAFriend_name']) : '';
							$ToEmail = isset($_POST['TellAFriend_email']) ? sanitize_text_field($_POST['TellAFriend_email']) : '';
							// echo $ToEmail;						

							$Link = sanitize_text_field($_POST['TellAFriend_Link']);
							$ToMessage = stripslashes(wp_filter_post_kses($_POST['TellAFriend_message']));
							...
							$Usermail_Content  = preg_replace('/%0D%0A/',' ',$Usermail_Content);
							$Usermail_Content  = preg_replace('/%3F%0D%0A%0D%0A/',' ',$Usermail_Content);
							$Usermail_Content  = preg_replace('/%E2%80%99/',' ',$Usermail_Content);
							$Usermail_Content  = preg_replace('/%0D%0A%0D%0A/','',$Usermail_Content);
							$Usermail_Content  = preg_replace('/%24/','$',$Usermail_Content);
							$Usermail_Content  = preg_replace('/%2C/',',',$Usermail_Content);
							$Usermail_Content  = preg_replace('/%3F/','.',$Usermail_Content);
