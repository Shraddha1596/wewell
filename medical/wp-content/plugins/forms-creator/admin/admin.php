<?php if ( ! defined( 'ABSPATH' ) ) exit; 
	...
	add_action('admin_menu', 'wpbiker_admin_menu_fp', 999);
	function wpbiker_admin_menu_fp() {
		add_menu_page(WPBIKER_FP_NAME, __( WPBIKER_FP_NAME, "wpbiker-fp-lang"), 'manage_options', WPBIKER_FP_BASENAME, 'wpbiker_admin_fp', 'dashicons-feedback', null);
		add_action('admin_print_styles', 'wpbiker_script_style');
	}
	
	function wpbiker_admin_fp() {
		global $wpbiker;	
		$wpbiker = true;	
		include_once( 'partials/form.php' );	
		wp_enqueue_script(WPBIKER_FP_BASENAME, plugin_dir_url(__FILE__) . 'js/script.js', array('jquery'));		
		wp_enqueue_style('wpbiker-style-custom', plugin_dir_url(__FILE__) . 'css/custom.css');
		wp_enqueue_style( 'font-awesome', WPBIKER_FP_URL . 'asset/font-awesome/css/font-awesome.min.css', array(), '4.7.0' );
